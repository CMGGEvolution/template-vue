...
- `Dsonar.javascript.lcov.reportPaths`: This parameter must be set to the path of the report file produced by your coverage tool. The path can be either absolute or relative to the project root. This doesn't work for me 🤫 but don't worry fi you want it,you will need to remove `coverage` folder inside the `.gitignore` as sonarCloud will need to to generate the coverage report. After that go to the `information` inside your project on `sonarCloud`, choose the badge `coverage` inside teh select and copy and paste the `markdown` link inside your `README.md` file.
